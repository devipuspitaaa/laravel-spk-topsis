...
@section('content')
	...
	<div class="panel panel-default">
		<div class="panel panel-body">
			<form method="post" action="/admin/penilaian/{{ $dataMotor->id }}/edit" autocomplete >
					...
					<div class="row">
						<div class="col-md-6 {{ $errors->has('krKonMesin') ? 'has-error' : ' ' }}">
							...
							<select name="krKonMesin" class="form-control">
								<option value=""></option>
								<option value="Sangat Baik" {{ $penilaianMotor->konMesin==='Sangat Baik' ? 'selected' : '' }}>Sangat Baik</option>
								<option value="Baik" {{ $penilaianMotor->konMesin==='Baik' ? 'selected' : '' }}>Baik</option>
								<option value="Cukup" {{ $penilaianMotor->konMesin==='Cukup' ? 'selected' : '' }}>Cukup</option>
							</select>
						...
						<div class="col-md-6 {{ $errors->has('krKonBody') ? 'has-error' : ' ' }}">
							...
							<select name="krKonBody" class="form-control">
								<option value=""></option>
								<option value="Sangat Baik" {{ $penilaianMotor->konBody==='Sangat Baik' ? 'selected' : '' }}>Sangat Baik</option>
								<option value="Baik" {{ $penilaianMotor->konBody==='Baik' ? 'selected' : '' }}>Baik</option>
								<option value="Cukup" {{ $penilaianMotor->konBody==='Cukup' ? 'selected' : '' }}>Cukup</option>
							</select>
				...
				<div class="form-group">
					<div class="row">
						<div class="col-md-6 {{ $errors->has('krLamaPakai') ? 'has-error' : ' ' }}">
							...
							<input type="text" name="krLamaPakai" class="form-control" value="{{ $penilaianMotor->thnPakai }}" maxlength="3">
						...
						<div class="col-md-6 {{ $errors->has('krHarga') ? 'has-error' : ' ' }}">
							...
							<input type="text" name="krHarga" class="form-control" value="{{ $penilaianMotor->harga }}">
							{!! $errors->first('krHarga','<p class=help-block>:message</p>') !!}
						</div>
					</div>
				</div>			

				<input type="hidden" name="krKonMesinLama" value="{{ $penilaianMotor->konMesin }}">
				<input type="hidden" name="krKonBodyLama" value="{{ $penilaianMotor->konBody }}">
				<input type="hidden" name="krLamaPakaiLama" value="{{ $penilaianMotor->thnPakai }}">
				<input type="hidden" name="krHargaLama" value="{{ $penilaianMotor->harga }}">
